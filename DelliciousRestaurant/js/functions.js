...
function playlist(id, type) {
	...
	if(type == 0) {
		...
		$('.tab-share,.tab-share2,.tab-embed').fadeOut();
	} else if(type == 1) {
...
function connect_modal2() {
	if($('#connect').is(':hidden')) {
		...
		// Add active class on tab
		$('#tab-register').addClass('modal-menu-item-active').siblings().removeClass('modal-menu-item-active');
	}
}
...
function share2(id, type) {
	...
	if(type == 0) {
		...
		// Add active class on tab
		$('#tab-share2').addClass('modal-menu-item-active').siblings().removeClass('modal-menu-item-active');
		...
		// Add attributes to social icons
		$('#fb-share').attr('onclick', 'doShare2(1, '+type+', '+id+')');
		$('#tw-share').attr('onclick', 'doShare2(2, '+type+', '+id+')');
		$('#gp-share').attr('onclick', 'doShare2(3, '+type+', '+id+')');
		$('#pn-share').attr('onclick', 'doShare2(4, '+type+', '+id+')');
		$('#em-share').attr('onclick', 'doShare2(5, '+type+', '+id+')');
	}
}
function doShare2(social, type, id) {
	// Social 1: Facebook
	// Social 2: Twitter
	// Social 3: Google+
	// Social 4: Pinterest
...
$(document).ready(function() {
	...
	$("#search").on('keyup', function(e) {
		...
		if(q == 0) {
			...
			$('.search-container').html('<div class="search-content"><div class="search-results"><div class="message-inner"><div class="retrieving-results">Entrain de chercher ...</div> <div class="preloader-left preloader-dark"></div></div></div></div>');
			var ms = 200;
		}
...
function hideModal() {
	$('#share,#share2, #playlist, #delete, #connect').fadeOut();
	$('.modal-background').fadeOut();
	$('.tab-share,.tab-share2.tab-embed,.tab-delete,.tab-login,.tab-register').fadeOut();
	$('#autoplay').prop('checked', false);
}
function hideSearch() {
